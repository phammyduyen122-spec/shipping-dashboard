...
async def main():
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            # Let's locate the ag-Grid select all checkbox in the header
            # We will print available elements to click
            print("Locating header checkboxes...")
            header_checkbox = page.locator(".ag-header-cell .ag-checkbox-input-wrapper, .ag-header-select-all, .ag-header-cell input[type='checkbox']").first
            if await header_checkbox.count() > 0:
                print("Clicking select-all header checkbox...")
                await header_checkbox.click(force=True)
                await page.wait_for_timeout(2000)
            else:
            ...
            target_path = "filtered_attributes_select_all.xlsx"
