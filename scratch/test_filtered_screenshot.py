...
async def main():
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            # Expand switcher for fresh food
            ...
            for cat in target_cats:
                ...
                for scroll_top in range(0, 800, 50):
                    ...
                    if await cat_elem.count() > 0 and await cat_elem.is_visible():
                        ...
                        break
                if not clicked:
                    print(f"Failed to find/click: {cat}")
                    
            print("Selected categories. Clicking 'Áp dụng' button...")
            submit_btn = page.locator(".ant-drawer-content button:has-text('Áp dụng')").first
            if await submit_btn.count() > 0:
                await submit_btn.click()
            else:
                print("Submit button 'Áp dụng' not found!")
                
            await page.wait_for_timeout(10000) # Wait for table reload
            
            # Print table row counts
            rows = page.locator("tbody tr")
            print(f"Found {await rows.count()} rows in the table.")
            
            # Print table inner text of first row if exists
            if await rows.count() > 0:
                print("First row text:")
                print(await rows.first.inner_text())
                
            screenshot_path = "C:/Users/DUYEN/.gemini/antigravity/brain/dca284e8-02b6-49ff-9cf8-e32c87a805c0/scratch/product_list_after_filter.png"
