...
for f in sorted(glob.glob("transfer_*.xlsx")):
    ...
    try:
        ...
        print(f"Auto-created to KFM rows: {len(kfm_dest_auto)}")
        if len(kfm_dest_auto) > 0:
            print("Sample rows:")
            # print code_col, to_col, orig_col, shipped_col, received_col, and 'Trạng thái'
            status_col = [c for c in df.columns if 'trạng thái' in c.lower()][0]
            print(kfm_dest_auto[[code_col, to_col, orig_col, shipped_col, received_col, status_col]].head(15))
